...
data = pd.read_csv(r'C:\Users\102400001744\Desktop\PERSONAL_FILES\CODING&STUFF\py\Practices&Exercises\Streamlit&Pandas\archive\HRDataset_v14.csv')
...
if len(department_data) > 0:
    ...
    # Filter the data by hour and create a bar chart
    start_hour = 8
    end_hour = 15
    hours = range(start_hour, end_hour)
    counts = [department_data[f"{h:1d}:00:00"].sum() for h in hours]
    plt.bar(hours, counts)
    plt.xlabel("Hour")
    plt.ylabel('')
    plt.title(f"Active Users per Hour for {department} Department")
    st.pyplot()
